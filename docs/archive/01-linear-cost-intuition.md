# 历史学习记录：张量、依赖与 Linear 的成本直觉

> 本文保留第一轮学习中的问题、推导和解答。公式中的已知笔误已经修正。
> 课程主线后来调整为“先理解语言模型怎样产生 token，再学习成本和优化”，
> 因此本文不再作为新版第一课；建议在完成新版第 4 课后，把它作为成本直觉补充阅读。
...
下一课将把单个 Linear 扩展为完整计算图，学习算子族、分支、汇合、残差、状态和前后处理。
