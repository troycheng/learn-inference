...
这一课补充后面理解语言模型时会反复用到的数学工具，范围不等同于一门完整的线性代数课：
...
学完后不要求熟练推导公式，但要能看出张量中每个维度表示什么，某个算子对哪些数做了什么，以及输出为什么会得到这样的 shape。
...
这些模块的完整作用会在后续课程中展开。这里先建立共同的数学语言，保证后面看到计算时，能说清谁和谁计算、沿哪个轴，以及输出 shape 为什么变化。
