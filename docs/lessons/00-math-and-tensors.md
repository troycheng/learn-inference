# 第 0 课：看懂推理链路所需的数学与张量

这一课不是线性代数课程。它只准备后续理解语言模型时会反复使用的工具：
...
学完后，你不需要熟练推导公式，但应该能够回答：一个张量中的每个维度表示什么，某个算子对哪些数做了什么，输出 shape 为什么是这样。
...
它们不是四种互不相关的东西，而是“数字按多少个轴组织”的不同情况。
...
两个结果在文字中都显示成一维列表，不能再用“横向还是竖向”判断。方向只存在于原矩阵；得到一维向量后，它只有一个轴。真正可靠的是看：哪个轴被消掉了，哪些轴被保留了。
...
`keepdim=true` 不会改变计算结果中的数，只保留一个长度为 1 的轴，方便后续广播。第二课的 RMSNorm 会使用这一点。
...
第二课会沿 `H` 轴计算每个 token 向量的均方值；第一课会沿词表 `V` 轴选择最大 Logit。这些看似不同的操作，都属于沿某个轴进行归约。
...
输出仍有三个元素，shape 仍是 `[3]`。第二课中的残差连接使用逐元素加法，SwiGLU 门控使用逐元素乘法。

## 7. 广播：小 shape 怎样参与大 shape 的逐元素计算
...
点积（Dot Product）解决的问题是：怎样把两组对应特征综合成一个数。
...
第一课的 LM Head 会用一次点积为一个候选 token 打分；第三课的 Attention 会用点积计算 Query 与 Key 的匹配分数。
...
可以利用矩阵乘法的结合律，预先把两套权重组合成一套等价权重。这里使用的是结合律，不是交换律。第二课会看到，SwiGLU 在两个 Linear 之间加入激活和逐元素门控，因此不能这样简单合并。

## 11. Embedding：按编号查表，不是矩阵乘法教学的起点
...
判断一个优化是在减少模型常驻容量，还是减少请求状态容量，必须先分清它改变了哪一类数据。

## 13. 把基础工具接回语言模型

下面这张表是第 0 课与后续课程的连接点。
...
你现在不需要知道这些模块的完整目的。第 0 课只保证后面出现计算时，能看懂“谁和谁计算、沿哪个轴、输出 shape 是什么”。
...
不要求背下所有公式。能够用小数字把计算重新走一遍，比背公式更重要。
