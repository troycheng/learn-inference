...
Decoder 内部、GPU、FLOPs 和服务吞吐暂时留在黑盒之外。这一课要先分清文字、Token ID、嵌入（Embedding）、隐藏状态（Hidden State）、未归一化分数（Logit）和概率，并把一次生成的完整链路接起来。

如果对 `[B,T,H]`、轴、点积或 Linear 的 shape 还不熟悉，先阅读[第 0 课：模型推理中的张量计算](00-math-and-tensors.md)。

本课程统一术语见[课程术语与符号表](../glossary.md)。

## 1. 从全局看：模型并不直接接收和输出文字
...
下面这张表列出了各组件在整条链路中的职责。后面的各节会逐一展开。
