...
先只看归一化本身。下面的例子忽略数值稳定项 `epsilon`，并令学习缩放 $\gamma=[1,1]$。

RMS 是“平方 → 平均 → 开平方”：

`[3,4]` 的平方是 `[9,16]`，均方根约为 `3.536`。两个元素分别除以它，得到 `[0.849,1.131]`。下图把这几步连在了一起：

![RMSNorm 的逐步计算](../assets/02-rmsnorm.svg?rev=20260809-1)
...
`H` 是模型各层共同约定的接口宽度，每一层都接收和输出 `H` 维 token 向量。`I` 只在 FFN 内部使用，表示门控和特征分支各自产生多少个中间特征。
...
这里的 `y_norm` 是经过 RMSNorm 的单个 token 向量，不是残差分支中未经归一化的 `y`。下图继续跟踪这条向量；为了能逐项手算，把真实模型的宽度缩小为 `H=2`，并取 `y_norm=[1,2]`。
...
继续计算上图中的 `y_norm=[1,2]`。它表示送进 FFN 的单个 token 向量，下面依次算出门控投影、扩展投影、逐元素相乘和回收投影。
