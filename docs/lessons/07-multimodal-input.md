...
$$
x_{patch}=pW_{patch}^{T}+b
$$

这与第 0 课的 Linear 是同一种写法：`W_patch` 的每一行与 `p` 做一次点积，产生一个输出坐标。同一组权重会用于图片中的每个 Patch。

先用一个 `4→3` 的缩小例子把计算走一遍：

![一个 Patch 怎样投影成一条向量](../assets/07-patch-embedding.svg)

图中第一行权重只读取左上角像素，所以第一个输出是 `0.2`；第二行把右上角与左下角相加，得到 `1.0`；第三行取右下角的一半，得到 `0.4`。真实权重不是人工指定的取值规则，而是在训练中学到的。这个例子只把矩阵缩小，计算方式没有改变。

Qwen3.5 的实际输入 `p:[1536]`，权重 `W_patch:[1152,1536]`，输出 `x_patch:[1152]`。概念上，可以把全部 Patch 写成 `[1024,1536]`。官方实现会在 Conv3d 前把它重排为 channel-first 的五维输入：
...
对纯文本位置，三组编号相同。只看图片块内部的局部坐标时，静态图片的时间编号保持不变，高度和宽度编号按合并后的 `16×16` 网格变化：
