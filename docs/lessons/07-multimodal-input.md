...
占位 Token ID 负责告诉处理器和模型“视觉向量放在这里”，真正描述图片内容的是替换进来的视觉向量。
...
9. 占位 Token ID 标记视觉向量应放的位置；真正描述图片的是视觉编码器输出。
...
第 8 课不再打开新算子，而是把前面学过的结构放回 `config.json`，练习从配置字段还原层数、参数量、缓存大小和一轮计算的大致数量级。
