...
## 12. 评审案例：P99 TTFT 超标后，三个方案该怎样取舍

本案例使用构造数据，不代表 Qwen3.5 的公开 benchmark。它模拟一次上线前技术评审：现有资料包括请求分布、同一条慢请求的 trace、GPU Profile 和两组局部测试。评审需要决定先验证哪个方案、暂缓哪个方案，以及还缺什么证据。
