...
## 12. 案例：P99 TTFT 超标的优化评审

以下数据专门用于教学，不是 Qwen3.5 的公开 benchmark。案例保留了线上评审中常见的四类证据：工作负载、SLO、分段 Profile 和备选方案的局部测试结果。

服务在一台 8 卡机器上运行 Qwen3.5-35B-A3B，当前部署为两个 TP=4 副本。模型、runtime、硬件和请求采样配置均已固定。工作负载和目标如下：
...
TPOT 已经达标，问题集中在首 token。长 Prompt Profile 还给出：Full Attention 占 Prefill GPU 时间的 24%，Gated DeltaNet 与 FFN 合计占 68%，其余算子占 8%。慢请求 trace 中还能看到明显的排队时间。

评审会上提出三个方案：升级 FlashAttention Kernel、开启 Prefix Cache、把两个 TP=4 副本改成一个 TP=8 副本。它们都可能有收益，但现有证据支持的结论并不相同。

### 12.1 FlashAttention 的收益上限

Microbenchmark 显示新的 Full Attention Kernel 快 1.8 倍。即使假设其他 Prefill GPU 计算完全不变，它对 Prefill GPU 时间的加速上限也只有：
...
这不表示新 Kernel 没有价值。它说明该方案的作用范围有限，是否值得上线要看实现成本、长 Prompt 占比以及它能否与其他方案叠加。

### 12.2 Prefix Cache 的收益对象

72% 的请求共享 4096 个精确 token，说明系统确实在重复执行大量 Prefill。Prefix Cache 与这段重复工作直接匹配，适合作为第一轮对照实验。

它仍不是已经确定的 P99 方案。未命中请求占 28%，远高于最慢的 1%；如果 P99 全部来自未命中或驱逐后的请求，命中路径即使大幅变快，总体 P99 也可能不变。另一方面，命中请求减少 GPU 占用后，队列缩短可能间接改善未命中请求。两种结果都要靠分桶和排队时间证明。

对照实验至少要分别报告：

| 请求分组 | 需要验证什么 |
| --- | --- |
| 命中请求 | 实际复用 token 数、TTFT、状态恢复正确性 |
| 未命中请求 | TTFT 是否退化、是否仍由完整 Prefill 或排队主导 |
| 驱逐后请求 | Cache 容量与驱逐是否制造新的尾延迟 |
| 全部请求 | TTFT P99、TPOT P99、同 SLO `goodput` 和队列时间 |

只有总体 P99 达标，或者新的分桶结果证明当前目标应改为另一类请求，才能形成上线结论。命中率本身不是业务结果。

### 12.3 TP=8 的收益与代价

TP=8 会让更多 GPU 共同计算一个请求，但也会把两个可独立接收请求的副本合成一个。团队已有的低并发测试显示：
...
这组数据只证明 TP=8 改善了低并发单请求 TPOT，同时增加了通信。它没有证明当前到达率下的 TTFT 会改善。当前 TPOT 已经达标，主要问题又是 Prefill 和排队；此时先牺牲一个副本，可能进一步降低排队容量。

所以 TP=8 不应排在本轮首位。若以后目标改成极低到达率下的单请求延迟，或者 TP=4 已经无法容纳模型和状态，再重新评估它。

### 12.4 当前评审结论

当前最合理的顺序是：

1. 先按共享前缀资格、真实命中、驱逐和排队时间拆分 P99 请求。
2. 在相同到达率下做 Prefix Cache 对照，因为它直接消除已经确认存在的重复 Prefill。
3. 若未命中长 Prompt 的 Prefill 仍是主要瓶颈，再评估 FlashAttention，并用实际 Profile 更新 Amdahl 上限。
4. 暂不把部署改为 TP=8；当前证据没有覆盖在线排队和同 SLO 吞吐。

这份材料只能确定实验顺序，还不能确定最终上线方案。第一轮补齐命中、未命中和排队证据后，再决定是继续优化 Prefix Cache、转向 FlashAttention，还是重新评估部署方式。
