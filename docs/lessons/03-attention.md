...
假设组 1 每经过一个 token 位置旋转 `30°`，组 2 每经过一个位置只旋转 `3°`。位置编号 `m` 决定累计旋转角度：

| 位置 `m` | 组 1：`m × 30°` | 组 2：`m × 3°` |
|---:|---:|---:|
| 0 | 0° | 0° |
| 1 | 30° | 3° |
| 2 | 60° | 6° |
...
当这个 token 位于位置 2 时，两组从相同的 `[1,0]` 出发，却转到了不同方向：

```text
组 1：[1,0] 旋转 60° → [cos 60°, sin 60°] ≈ [0.500, 0.866]
组 2：[1,0] 旋转  6° → [cos  6°, sin  6°] ≈ [0.995, 0.105]

旋转后的 q' ≈ [0.500, 0.866, 0.995, 0.105]
```

这个例子中的角度是为了方便观察而设定的。真实 RoPE 使用弧度，并按公式为各个二维组生成一系列不同的频率。
