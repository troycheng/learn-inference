...
这里沿用上一节显示到小数点后三位的权重，所以矩阵第 3 行与手算结果相同。它就是“它”取回的信息。完整流程如下：
...
![Qwen3.5 的 GQA 分组](../assets/03-gqa.svg?rev=20260809-1)
