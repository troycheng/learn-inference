...
先明确符号。用 `m` 表示当前 Q 的位置，用 `n` 表示被读取的 K 的位置。对于 Decoder，通常有 `n≤m`。

在一个二维特征组中，假设每前进一个位置旋转 `θ`。`R_p` 表示旋转 `pθ`，并不是数字 `p`：
...
这里的“速度”不是计算快慢，而是位置每增加 1，这个二维组增加多少旋转角度。第 `i` 组的单位旋转角度记作 `θᵢ`，token 位于位置 `m` 时，这一组累计旋转 `mθᵢ`。

下面用一个四维向量说明。它被分成两个二维组，两组读取同一个位置编号 `m=2`。第一组每个位置增加 30°，所以转到 60°；第二组每个位置增加 3°，只转到 6°。

![同一位置在不同二维组中的旋转结果](../assets/03-rope-frequencies.svg?rev=20260811-1)

如果所有组都采用同一个 `θ`，它们会重复相同的位置变化。采用不同的 `θᵢ` 后，同一个相对距离 `Δ` 会同时形成 `Δθ₁、Δθ₂……` 多个角度。转得快的组变化明显，转得慢的组变化较缓，各组也较少同时转回起点。Attention 可以结合这些结果判断位置关系。
