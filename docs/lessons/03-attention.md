...
Qwen3.5 的 Token Mixer 有两种。Gated DeltaNet 把读过的信息持续整理进固定大小的状态；Full Attention 让当前 token 直接与所有可见 token 比较。这一课只拆 Full Attention。
...
`Full` 说的是比较范围。序列有 `T` 个 token 时，一个 Attention 头在概念上会得到一张 `T×T` 的分数表：每一行代表一个正在更新的 token，每一列代表一个可供它读取的 token。由于 Decoder 不能偷看未来，每一行只能使用当前位置及其左侧部分。Gated DeltaNet 放到第 5 课。
