...
取一个可以手算的特例。令原始 `q=k=[1,0]`，每前进一个位置旋转 30 度。当前位置的 Q 在位置 3，旋转 90 度；它读取的历史 K 在位置 2，旋转 60 度。两根箭头相差 30 度，点积是 `cos(30°)≈0.866`。把它们一起移到位置 13 和位置 12，点积仍然不变。

这个特例只说明共同移动不会改变相对位置。真实 Q、K 的内容不同，维度也更高。Attention 分数不是一张只按距离查询的固定表。

### 6.2 R 矩阵怎样作用到真实 Q/K

上一节只画了一根二维箭头。真实计算中，一个 Q 或 K 头有很多维。Qwen3.5-9B 每头共 `256` 维，其中前 `64` 维参与 RoPE，后 `192` 维原样通过：
...
`q_rot` 和 `k_rot` 各自再切成等长的前后两半。以 Q 为例：
...
因此，Qwen3.5 代码中的一组不是相邻的 `(a0,a1)`，而是前后两半中索引相同的 `(a0,b0)`。K 采用相同的拆法。

每组都有自己的单位旋转角度。对于纯文本，Qwen3.5-9B 使用 `d_rot=64`、`B=10,000,000`。第 `i` 组的单位角度为：

$$
\theta_i=B^{-2i/d_{rot}},\qquad i=0,1,\ldots,31
$$

`θᵢ` 的单位是弧度。token 位于位置 `p` 时，第 `i` 组实际旋转的角度是：
...
$$
\begin{aligned}
a_i'&=a_i\cos(\phi_{p,i})-b_i\sin(\phi_{p,i})\\
b_i'&=a_i\sin(\phi_{p,i})+b_i\cos(\phi_{p,i})
\end{aligned}
$$

Qwen3.5 用 [`rotate_half`](https://github.com/huggingface/transformers/blob/943628458a1691f8af09c47ea9fc6e314734722f/src/transformers/models/qwen3_5/modeling_qwen3_5.py#L546-L589) 一次并行处理全部 32 组：
...
这里两个乘法都是逐元素乘法。代码预先算好每组的 cos 和 sin，不会创建 32 个 `2×2` 矩阵。

下面把旋转区缩小到 4 维，但保留同样的前后半区布局。输入取 `q_rot=[a0,a1 | b0,b1]=[1,1 | 0,0]`，位置取 `p=2`。为了看清两组的差别，示例令 `θ0=30°`、`θ1=3°`；图中算出两组结果，再按 `[a0',a1' | b0',b1']` 的顺序放回。

![Qwen rotate_half 的四维缩小示例](../assets/03-rope-frequencies.svg?rev=20260811-2)

真实的 64 维旋转区只是把上面的两组扩大为 32 组。后面的 192 维不参与这些计算，最后直接拼回 Q/K。

### 6.3 两次旋转怎样合成 `R_{n-m}`

下面只看第 `i` 组，并暂时省略下标 `i`，把 `R_{p,i}` 简写成 `R_p`。用 `m` 表示当前 Q 的位置，用 `n` 表示被读取的 K 的位置。对于 Decoder，通常有 `n≤m`。

接下来只需要旋转矩阵的两条性质：

1. 转置会把旋转方向反过来：`R_m^T=R_{-m}`。
2. 连续旋转时，角度可以相加：`R_aR_b=R_{a+b}`。
...
`R_{n-m}` 的含义是“旋转 `(n-m)θ`”，不是拿两个矩阵做 `R_n-R_m`。

仍用上面的数字：Q 在位置 `m=3`，历史 K 在位置 `n=2`，每步旋转 `θ=30°`：
...
直接把 Q 转 90 度、K 转 60 度再做点积，也会得到 `0.866`。`R_{n-m}` 只是把这两次绝对位置旋转，化简成一次相对位置旋转。
...
实际代码通常不会先构造一个名为 `R_{n-m}` 的矩阵。它使用位置 `m` 的正弦、余弦旋转 Q，使用位置 `n` 的正弦、余弦旋转 K；两者做点积后，数学效果等价于上面的 `R_{n-m}`。

负号还保留了方向。在这个例子中，`n-m=-1` 表示 K 位于 Q 左边一个位置。不同资料可能采用相反的旋转或行列向量约定，因而写成 `m-n`；关键是最终只剩两个位置之差。
...
这里的 `R_m` 是简写，代表 32 组 `R_{m,i}` 一起作用于前 64 维，后 192 维原样通过。它不是上一节中某一组的 `2×2` 矩阵。撇号只表示“准备好进入 Attention 的数据”。V 不参与 Q/K 打分，所以不旋转。
...
这里的“速度”不是计算快慢，而是公式中的 `θᵢ`：位置每增加 1，第 `i` 组增加多少旋转角度。随着组号 `i` 增加，Qwen3.5 公式生成的 `θᵢ` 逐渐减小，所以 32 组会以不同速度旋转。

如果所有组都使用同一个 `θ`，32 组会重复相同的位置变化。使用不同的 `θᵢ` 后，同一个相对距离 `Δ` 会同时形成 `Δθ₀、Δθ₁……Δθ₃₁`。有的角度变化较快，有的变化较慢，各组也较少同时回到起点。Attention 可以结合这 32 组结果判断位置关系。

“快组看近处、慢组看远处”只能作为直觉，模型没有预先规定每组负责哪种距离。RoPE 也不保证邻近 token 一定得到更高权重，分数仍由 Q/K 内容和相对位置共同决定。

Qwen3.5 使用 MRoPE。上面的公式先按纯文本的一维位置理解；图片和视频的时间、高度、宽度坐标放到第 7 课。

### 6.6 RoPE 为什么只作用于 Q 和 K
