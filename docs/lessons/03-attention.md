...
这里的 `1`、`2`、`3` 是手算示例的局部编号，不是开头七位置句子的原始编号：原位置 1“小明”映射为示例位置 1，原位置 3“杯子”映射为示例位置 2，原位置 7“它”映射为示例位置 3。后文紧接这个手算时说的“位置 3”，都指示例中的“它”。
...
这三个数都显示到小数点后三位。Softmax 先用未截断的缩放分数计算，再四舍五入展示，因此上面显示值相加为 `1.001` 只是舍入误差；未截断权重的和为 1。
...
单个查询头的整段输出是 `[B,T,D]`；把全部 `Nq` 个单头输出堆在一起，才得到中间张量 `[B,Nq,T,D]`。接下来把头轴和头内维度拼回去：
