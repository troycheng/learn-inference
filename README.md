...
- [综合评审：Qwen3.5-9B 的容量与优化方案](docs/capstone.md)
...
第一次阅读只需追踪数据表示、shape 变化和请求状态，不必记住 Qwen3.5 的每个配置数字。遇到公式时，可以运行对应的复算程序。每课末尾的实践题用于检查能否把同一个原理应用到一组新数据。

读完第 9 课后，完成一次[综合评审](docs/capstone.md)：根据给定的模型配置和线上症状，提交架构摘要、容量计算和优化上线结论。参考分析最后给出了一页技术结论示例。随后可以换成自己正在部署的模型；每个数字都应能追溯到配置、源码或测量结果。
